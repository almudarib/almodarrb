 ...
 * - تعتمد على وجود student_id في الكوكيز لتحديد هوية الطالب
...
export default async function StudentDashPage() {
    ...
    return (
      <div className="min-h-svh bg-slate-50 flex items-center justify-center p-6">
        <div className="w-full max-w-xl bg-white rounded-2xl border border-slate-200 p-6 shadow-sm text-center">
          ...
          <Link className="inline-flex items-center px-4 py-2 rounded-lg bg-sky-600 text-white hover:bg-sky-700" href="/student/login">
  ...
  const [examsR, sessionsR, doneExamsR, doneSessionsR] = await Promise.all([
    ...
    countStudentCompletedExams(studentId, language),
    countWatchedSessionsForStudent(studentId, language),
  ]);
   ...
   * - يحذف الكوكيز الخاصة بالطالب ويوجه إلى صفحة الدخول
   */
  async function logoutAction() {
    'use server';
    const cookieStore = await cookies();
    const sid = parsePositiveInt(cookieStore.get('student_id')?.value ?? '');
    try {
      if (sid) {
        try {
          await logStudentLogout({ student_id: sid });
        } catch {}
        try {
          await finishLastAppSessionForStudent(sid);
        } catch {}
      }
    } finally {
      try {
